      ...
      <tbody>
        {sortedTeams.map((team, index) => (
          <tr key={team.id} className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800'} border-b dark:border-gray-700`}>
            ...
            <td className={`py-0 sm:py-1 px-1 sm:px-2 ${showPos ? 'w-40 sm:w-60' : 'w-16'}`}>
              <div className="flex items-center h-6 sm:h-7">
                ...
                <span className="font-semibold inline sm:hidden ml-1 pl-0.5 truncate">{team.abbr || team.name?.substring(0, 3)}</span>
              </div>
            </td>
            ...
            {sortedUsers.map(user => {
              ...
              const colorClass = getCellColorClass(predictedPosition, team.actualPosition, points);

              if (!showPoints) {
                return (
                  <td key={user.id} className={`text-center p-0 border-l border-gray-300/30 font-semibold w-8 ${colorClass}`}>
              ...
              return (
                <React.Fragment key={user.id}>
                  <td className={`text-center p-0 border-r border-gray-300/30 font-semibold w-8 ${colorClass}`}>
                    <div className="h-full w-full py-1 px-0 flex items-center justify-center text-[11px] sm:text-sm">
                      {typeof predictedPosition === 'number' ? predictedPosition : '-'}
                    </div>
                  </td>
                  <td className={`text-center p-0 border-gray-300/30 font-semibold w-8 ${colorClass}`}>
                    <div className="h-full w-full py-1 px-0 flex items-center justify-center text-[11px] sm:text-sm">
                      {points}
                    </div>
                  </td>
        ...
        <tr className="bg-gray-300 dark:bg-gray-700 font-bold">
          ...
          {sortedUsers.map(user => (
            <td
              ...
              colSpan={showPoints ? 2 : 1}
              className="text-center py-0 px-2 border-l-2 border-slate-100 dark:border-gray-600"
            >
              {user.points}
            </td>
          ))}
